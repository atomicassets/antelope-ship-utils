...
# The 1.2.0 line: one CHANGELOG entry, a candidate tag, then the stable tag.
cat >CHANGELOG.md <<'EOF'
# Changelog
...
if [ "$RUN_RC" -eq 0 ] &&
    ...
    ok "v1.1.0 drops the entry heading, promotes the section, lists $want commits, and ends with the compare link"
else
...
if [ "$RUN_RC" -ne 0 ] &&
    ...
    ok "a version with no CHANGELOG entry exits non-zero and names the version"
else
...
# --- 5: a non-v tag is refused ------------------------------------------------

run 1.0.0
if [ "$RUN_RC" -ne 0 ] && printf '%s\n' "$RUN_OUT" | grep -q 'not a v-prefixed tag'; then
    ok "a bare semver tag is refused because this repository tags with a v prefix"
else
    no "non-v tag" "rc=$RUN_RC out='$RUN_OUT'"
fi
