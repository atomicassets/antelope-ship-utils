#!/usr/bin/env bash
# Compose the GitHub Release body for a released tag: the CHANGELOG entry for
# the version, the commit list for the tag range, and the compare link.
#
...
case "$TAG" in
    v[0-9]*) ;;
    *) die "tag $TAG is not a v-prefixed tag" ;;
esac
# A prerelease reads the entry for the version it is a candidate for, so
# vX.Y.Z-rc.1 and vX.Y.Z compose from one CHANGELOG entry.
VERSION="${TAG#v}"
VERSION="${VERSION%%-*}"
...
# The heading match is a prefix, so trailing text such as a date is ignored.
# sed drops the blank lines above the first line of content, and the command
# substitution ($(...)) drops the trailing ones.
ENTRY="$(git show "$TAG:CHANGELOG.md" | awk -v h="## [$VERSION]" '
    !inside && ($0 == h || index($0, h " ") == 1) { inside = 1; next }
    inside && /^## / { exit }
    inside { print }
' | sed '/./,$!d')"
[ -n "$ENTRY" ] || die "CHANGELOG.md at $TAG has no \"## [$VERSION]\" entry"
